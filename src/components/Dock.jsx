...
// Adaptado de um componente shadcn/TSX (framer-motion) para Vite + React +
// JavaScript, usando `motion/react` (mesma API, pacote atual/sucessor).
  ...
  return (
    <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-end overflow-x-auto">
      <motion.div
        ...
        className={cn('mx-auto flex w-fit gap-4 rounded-2xl border border-aje-cafe/20 bg-aje-paper px-4 shadow-sm', className)}
...
export function DockItem({ children, className, href, onClick }) {
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={cn(
            'absolute -top-7 left-1/2 w-fit whitespace-pre rounded-md border border-aje-cafe/20 bg-aje-paper px-2 py-0.5 text-xs text-aje-ink shadow-sm',
            className
          )}
          role="tooltip"
          style={{ x: '-50%' }}
