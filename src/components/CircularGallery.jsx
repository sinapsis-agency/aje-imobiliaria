...
// Adaptado de um componente shadcn/TSX (biblioteca `ogl`, WebGL) para Vite +
// React + JavaScript puro — apenas removendo anotações de tipo do
// TypeScript; a lógica e os shaders são idênticos ao original.
...
class Media {
  ...
  createShader() {
    ...
    img.onerror = () => {
      if (this.image.fallback && img.src !== this.image.fallback) {
        img.src = this.image.fallback;
      }
    };
...
export function CircularGallery({
  ...
  useEffect(() => {
    ...
    const computedColor = computedStyle.color || '#1a1512';
  ...
  return (
    <div
      ref={containerRef}
      className={`h-full w-full cursor-grab overflow-hidden text-aje-ink active:cursor-grabbing ${className}`}
      style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '20px' }}
      {...props}
    />
  );
}
